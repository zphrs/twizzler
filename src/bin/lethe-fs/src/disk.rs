...
impl Read for Disk {
    fn read(&mut self, buf: &mut [u8]) -> Result<usize, Self::Error> {
        ...
        while bytes_written != buf.len() {
            ...
            let left_page_index = self.pos % PAGE_SIZE;
            let right_page_index = if left_page_index + buf.len() - bytes_written > PAGE_SIZE {
                PAGE_SIZE
            } else {
                left_page_index + buf.len() - bytes_written
            }; // If I want to write more than the boundary of a page
            block_on(self.ctrl.read_page(lba as u64, &mut read_buffer, 0));
            let key = self.khf.derive(lba as KhfKeyId);

            let bytes_to_read = right_page_index - left_page_index;

            buf[bytes_written..bytes_written + bytes_to_read]
                .copy_from_slice(&read_buffer[left_page_index..right_page_index]);

...
use nvme::{
    ...
    ds::{
        ...
        queue::{
            comentry::CommonCompletion, subentry::CommonCommand, CommandId,
            QueueId, QueuePriority,
        },
    },
...
pub async fn init_controller(ctrl: &mut Arc<NvmeController>) {
    ...
    let aqa =
        nvme::ds::controller::properties::aqa::AdminQueueAttributes::new()
...
impl NvmeController {
    ...
    async fn create_queue_pair(
        ...
        let bell_stride: usize =
            map_field!(reg.capabilities).read().doorbell_stride_bytes();
        let _ = 0;
    ...
    pub async fn identify_namespace(
        ...
        let nslist_cmd = nvme::admin::Identify::new(
            CommandId::new(),
            nvme::admin::IdentifyCNSValue::ActiveNamespaceIdList(
                NamespaceId::default(),
            ),
            (&mut nslist)
                .get_dptr(
        ...
        let ident_cmd = nvme::admin::Identify::new(
            CommandId::new(),
            nvme::admin::IdentifyCNSValue::IdentifyNamespace(NamespaceId::new(
                1u32,
            )),
            (&mut ident)
                .get_dptr(
    ...
    pub async fn flash_len(&self) -> u64 {
        if let Some(sz) = self.capacity.get() {
            *sz
        } else {
            self.identify_controller().await;
            let ns = self.identify_namespace().await;
            let block_size: u64 = ns.lba_formats()
                [ns.formatted_lba_size.index()]
            .data_size() as u64;
            // don't care if this set went through
            let _ = self.capacity.set(block_size * ns.capacity);
            block_size * ns.capacity
        }
    }
    ...
    pub async fn write_page(
        ...
        buffer.with_mut(|data| {
            data[offset..(offset + len)].copy_from_slice(in_buffer)
        });
    ...
    pub async fn get_lba_size(&self) -> usize {
        if let Some(sz) = self.block_size.get() {
            *sz
            ...
            let ns = self.identify_namespace().await;
            let block_size =
                ns.lba_formats()[ns.formatted_lba_size.index()].data_size();
            // don't care about whether this set is the actual current value

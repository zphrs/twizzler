...
use twizzler_driver::dma::{
    DeviceSync, DmaPin, DmaPool, DmaRegion, DmaSliceRegion, DMA_PAGE_SIZE,
};
...
fn __get_prp_list_or_buffer(
    ...
    let prp = match pin_len {
        ...
        _ => {
            let mut first_prp_page =
                dma.allocate_array(entries_per_page, 0u64).unwrap();
        ...
        match self.mode {
            PrpMode::Double => {
                if self.buffer {
                    ...
                    PrpListOrBuffer::PrpFirstAndList(
                        self.embed[0],
                        self.embed[1],
                    )
                }
...
impl<'a, T: DeviceSync> PhysicalPageCollection
    for &'a mut NvmeDmaSliceRegion<T>
{
    fn get_prp_list_or_buffer(
        &mut self,
        mode: PrpMode,

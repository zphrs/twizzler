// no-ops all of the WAL since we don't have stdfs
// yet
...
pub struct SecureWAL<T, G, C, const N: usize> {
    /// The path to the WAL.
    // path: PathBuf,
...
unsafe impl<T, G, C, const N: usize> Sync for SecureWAL<T, G, C, N> {}

impl<T, G, C, const N: usize> SecureWAL<T, G, C, N> {
    /// Opens a new WAL.
    pub fn open(
        path: impl AsRef<Path>,
        key: Key<N>,
    ) -> Result<Self, SecureWALError<G::Error, C::Error>>
    where
    ...
    pub fn clear(&self) -> Result<(), SecureWALError<G::Error, C::Error>>
    where
        G: Ivg,
        C: StatefulCrypter,
    {
    ...
    pub fn persist(&self, key: Key<N>) -> Result<(), SecureWALError<G::Error, C::Error>>
    where
        T: Serialize + Sized,
        G: Ivg,
        C: StatefulCrypter,
    ...
    /// Each WAL entry is laid out as: [len, iv, serialization, checksum].
    fn load(&self, key: Key<N>) -> Result<(), SecureWALError<G::Error, C::Error>>
    where
        ...
        C: StatefulCrypter,
    {
        ...
        //     wal_len += mem::size_of_val(&len) + iv.len() + ser.len() +
        // mem::size_of_val(&checksum); }
    ...
    pub fn iter_persisted(&self, key: Key<N>) -> Result<IterPersisted<T, G, C, N>, io::Error>
    where
        T: for<'de> Deserialize<'de>,
    {
        Ok(IterPersisted {
...
impl<'a, T, G, C, const N: usize> IntoIterator for &'a SecureWAL<T, G, C, N>
where
    T: Clone,
{
...
pub struct IterPersisted<'a, T, G, C, const N: usize> {
...
impl<T, G, C, const N: usize> Iterator for IterPersisted<'_, T, G, C, N>
where

use std::{cmp::Ordering, collections::HashSet, fmt, marker::PhantomData, path::Path};
...
use crate::{
    ...
    crypter::{self, Ivg, StatefulCrypter},
    hasher::Hasher,
...
pub type KhfKeyId = u128;

/// The default level for roots created when mutating a `Khf`.
pub const DEFAULT_ROOT_LEVEL: KhfKeyId = 1;
/// The default fanout list for a `Khf`.
pub const DEFAULT_FANOUTS: &[KhfKeyId] = &[4, 4, 4, 4];
...
pub enum Consolidation {
    ...
    AllLeveled { level: KhfKeyId },
    /// Consolidate a range of keys with L1 subroots.
    Ranged { start: KhfKeyId, end: KhfKeyId },
...
pub struct Khf<
    R,
    G: Ivg + std::default::Default,
    C: StatefulCrypter + std::default::Default,
    H,
    const N: usize,
> {
    ...
    pub(crate) keys: KhfKeyId,
    // The number of keys covered currently.
    pub(crate) in_flight_keys: KhfKeyId,
    // The list of roots.
...
pub struct KhfBuilder<R, G, C, H, const N: usize> {
    fanouts: Vec<KhfKeyId>,
    fragmented: bool,
    _pd: PhantomData<(R, G, C, H)>,
}

impl<R, G: Ivg, C: StatefulCrypter, H, const N: usize> KhfBuilder<R, G, C, H, N> {
    pub fn new() -> Self {
        Self {
    ...
    pub fn fanouts(&mut self, fanouts: &[KhfKeyId]) -> &mut Self {
    ...
    > Khf<R, G, C, H, N>
{
    ...
    pub fn with_fanouts(fanouts: &[KhfKeyId]) -> Self
    where
        R: KeyGenerator<N> + Default,
        G: Default,
        C: Default,
    ...
    pub fn num_keys(&self) -> KhfKeyId {
        self.in_flight_keys
    }

    /// Returns the number of roots in the `Khf`'s root list.
    pub fn num_roots(&self) -> KhfKeyId {
        self.roots.len() as KhfKeyId
    }
    ...
    pub fn consolidate(&mut self, mechanism: Consolidation) -> Vec<KhfKeyId>
    where
        R: KeyGenerator<N>,
        H: Hasher<N>,
    {
    ...
    pub fn consolidate_all(&mut self, root: Node<H, N>) -> Vec<KhfKeyId>
    where
        H: Hasher<N>,
    {
    ...
    pub fn consolidate_all_leveled(&mut self, level: KhfKeyId, root: Node<H, N>) -> Vec<KhfKeyId>
    where
        H: Hasher<N>,
    {
    ...
    pub fn derive_inner(&mut self, key_id: KhfKeyId) -> Option<Key<N>>
    where
        H: Hasher<N>,
    {
    ...
    pub fn mark_key_inner(&mut self, key_id: KhfKeyId) {
        self.in_flight_keys = self.in_flight_keys.max(key_id + 1);
    }

    pub fn derive_mut_inner(&mut self, key_id: KhfKeyId) -> Key<N>
    where
        H: Hasher<N>,
    {
        let pos = self.topology.leaf_position(key_id);
    ...
    pub fn delete_key_inner(&mut self, key_id: KhfKeyId) -> bool {
    ...
    pub fn truncate_keys(&mut self, keys: KhfKeyId)
    where
        H: Hasher<N>,
    {
    ...
    pub fn updated_key_id_ranges(
        &self,
        updated_keys: &HashSet<KhfKeyId>,
    ) -> Vec<(KhfKeyId, KhfKeyId)> {
    ...
    pub fn update_keys(&mut self, level: KhfKeyId, start: KhfKeyId, end: KhfKeyId, root: Node<H, N>)
    where
        H: Hasher<N>,
    {
    ...
    pub fn update_inner(&mut self, updated_keys: &HashSet<KhfKeyId>) -> Vec<(KhfKeyId, Key<N>)>
    where
        R: KeyGenerator<N>,
        H: Hasher<N>,
    {
        ...
        match (self.in_flight_keys, self.keys) {
            ...
            (in_flight_keys, keys)
                if in_flight_keys >= keys && res.len() as KhfKeyId >= in_flight_keys =>
            {
            ...
            (in_flight_keys, _keys) if res.len() as KhfKeyId >= in_flight_keys => {
...
where
    ...
    H: Hasher<N>,
{
    fn persist(&mut self, _root_key: Key<N>, _path: impl AsRef<Path>) -> Result<(), Self::Error> {
        todo!();
...
where
    ...
    C: StatefulCrypter + std::default::Default,
{
    fn new(khf: &'a mut Khf<R, G, C, H, N>, start_key_id: KhfKeyId, end_key_id: KhfKeyId) -> Self {
...
pub struct RangedDeriveMutIter<
    'a,
    R,
    G: Ivg + std::default::Default,
    C: StatefulCrypter + std::default::Default,
...
where
    H: Hasher<N>,
    G: crypter::Ivg + std::default::Default,
    C: crypter::StatefulCrypter + std::default::Default,
{
    fn new(khf: &'a mut Khf<R, G, C, H, N>, start_key_id: KhfKeyId, end_key_id: KhfKeyId) -> Self {
        let leaf_pos = khf.topology.leaf_position(start_key_id);
...
pub struct KhfStats {
    pub keys: KhfKeyId,
    pub in_flight_keys: KhfKeyId,
    pub num_roots: KhfKeyId,
}
...
mod tests {
    ...
    impl KhfHarness {
        ...
        fn with_test_name_and_fanouts(name: &str, fanouts: &[KhfKeyId]) -> Self {
            let wal_path = Self::wal_path(name);
        ...
        fn with_test_name_and_fanouts_fragmented(name: &str, fanouts: &[KhfKeyId]) -> Self {
            let wal_path = Self::wal_path(name);
    ...
    fn update() -> Result<()> {
        ...
        for (key_id, key) in keys.iter().enumerate() {
            assert_eq!(update.get(&(key_id as KhfKeyId)), Some(key));
        }

        // We shouldn't have the old keys anymore.
        for (key_id, key) in keys.iter().enumerate() {
            assert_ne!(khf.derive(key_id as KhfKeyId)?.unwrap(), *key);
        }
    ...
    fn delete() -> Result<()> {
        ...
        for (key_id, key) in keys.iter().enumerate() {
            if key_id == 3 {
                ...
                assert_ne!(khf.derive(key_id as KhfKeyId)?.unwrap(), update[0].1);
            } else if key_id == 7 {
                assert!(khf.derive(key_id as KhfKeyId)?.is_none());
            } else {
                assert_eq!(khf.derive(key_id as KhfKeyId)?.unwrap(), *key);
            }
        }
    ...
    fn ranged_derive_mut_fragmented() -> Result<()> {
        ...
        let mut keys: BTreeMap<_, _> = (0..num_test_keys)
            ...
            .collect();
        assert_eq!(keys.len() as KhfKeyId, num_test_keys);
        wal.clear()?;

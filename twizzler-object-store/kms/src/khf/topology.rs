...
impl HeapSize for Topology {
    fn heap_size(&self) -> usize {
        self.descendants.heap_size()
    }
}

impl Topology {
    pub fn new(fanouts: &[KhfKeyId]) -> Self {
        let mut leaves = fanouts.iter().product();
        let mut descendants: Vec<u128> = Vec::with_capacity(fanouts.len() + 2);
    ...
    pub fn height(&self) -> KhfKeyId {
        self.descendants.len() as KhfKeyId
    }

    pub fn fanout(&self, level: KhfKeyId) -> KhfKeyId {
        if level == 0 {
            0
        } else if level == self.height() as KhfKeyId {
            1
        } else {
    ...
    pub fn descendants(&self, level: KhfKeyId) -> KhfKeyId {
        self.descendants[level as usize]
    }

    pub fn start(&self, node: Pos) -> KhfKeyId {
        if node.0 == 0 {
            0
        } else {
    ...
    pub fn end(&self, node: Pos) -> KhfKeyId {
        if node.0 == 0 {
            0
        } else {
    ...
    pub fn offset(&self, leaf: KhfKeyId, level: KhfKeyId) -> KhfKeyId {
        if level == 0 {
            0
        } else {
    ...
    pub fn leaf_position(&self, leaf: khf::KhfKeyId) -> Pos {
        (self.height() - 1, leaf)
    }
    ...
    pub fn coverage(&self, level: KhfKeyId, start: KhfKeyId, end: KhfKeyId) -> Coverage<'_> {
...
impl<'a> Coverage<'a> {
    pub fn new(topology: &'a Topology, level: KhfKeyId, start: KhfKeyId, end: KhfKeyId) -> Self {
